...
def filter_known_snp(infile, known, outfile):
    ...
    eff_df = pd.read_csv(infile, sep='\t', comment='#', names=names1, dtype=names1_dtype, index_col=False)
    ...
    if known.endswith('.gz'):
        snp_df = pd.read_csv(known, sep='\t', compression='gzip', names=names2 , dtype=names2_dtype)
    else:
        snp_df = pd.read_csv(known, sep='\t', names=names2, dtype=names2_dtype)
    ...
    with open(outfile, 'a') as o:
        keep[names1[:10]].to_csv(o, header=False, index=False, sep='\t')
